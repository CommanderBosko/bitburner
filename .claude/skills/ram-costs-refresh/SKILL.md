...
6. Present the diff to the user, then confirm via the **AskUserQuestion** tool — options `Apply diff and update ram-costs.json` / `Cancel` — before writing; don't fold this into free-form prose. This touches every entry in a file every other RAM-cost-aware skill in this repo trusts, so it needs the same explicit gate `ns-cost-lookup` already uses for its much smaller single-key adds. On confirmation, Edit `ram-costs.json`: update changed values, add newly-resolved entries, update `__source__`'s fetched-date, and append a dated sentence to `__note__` in the same running-changelog style as its existing entries (what changed, and why).
