...
4. **Patch the missing edge case.** `augment-loop.ts`, `faction-work-loop.ts`, `backdoor-loop.ts`, `controller.ts`, and `project-state.md` are known hot files that drift mid-session (see the `feedback-regrep-before-edit-hot-files` memory note) — re-grep the exact `old_string` immediately before calling Edit rather than trusting an earlier Read, even one from earlier in this same conversation.
