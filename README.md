...
- **Full audit sweep of the `.claude/skills/` catalog** — all 14 project-local skills checked against a quality rubric, 5 correctness bugs fixed (stale boot-chain/dispatch descriptions, a drifted file list, a stale commit count, an undocumented RAM-auditor blind spot), plus two cross-cutting duplications de-duplicated into shared config/assets. No game-code changes.
