...
- Fixed four stacked bugs behind scripted income staying ~10x below Hacknet even after prior fixes got it off $0: `home-upgrade-loop.ts`/`server-purchase-manager.ts` now get a small guaranteed RAM reserve (they were being starved indefinitely, freezing fleet growth); the batch fair-share budget no longer counts structurally-unfundable candidates when splitting RAM among working-set targets; `dispatchBatch` now packs largest-thread-count-first (first-fit-decreasing) so a fleet of mostly small purchased servers doesn't fragment RAM out from under its own batches; `buildWorkingSet`'s admission math now accounts for real per-host thread-rounding loss instead of a raw whole-pool-total estimate. Confirmed live: `hack.js` running, money climbing.
