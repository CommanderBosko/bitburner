...
- **The augment purchaser's "NeuroFlux Governor only" bug got its 3rd fix** — `canBuyNfg` treated "no purchasable candidates" as "nothing real left to ever buy," but that's also true for a real augmentation that's simply still rep-gated. Now the gate also checks that count is zero. Combined with the 2026-08-11 fix (rotate reputation-earning across every joined faction instead of committing to one forever), this closes the loop on two independent bugs that produced the same symptom.
- **`augment-loop.ts` now donates spare money to close faction reputation gaps** via `donateToFaction`, instead of leaving it idle — closest-gap-first, skipping the player's own gang faction. Also fixed an install-ordering bug this surfaced: a donation that unlocks an augmentation doesn't get spent on it until the next tick, so a donate-only tick now blocks that tick's install too (otherwise it would wipe reputation before the augmentation was ever bought).
...
- **`gang-manager.ts` split from a 36.10GB monolithic script into a ~3.60GB orchestrator + 7 transient `gang-agent-*.ts` workers** (mirroring `corp-manager.ts`'s pattern) — the old version referenced every `ns.gang.*` function directly and permanently starved itself out of a 64GB home. Confirmed live via `mem` (exactly 3.60GB, matching prediction).
