...
- `src/scripts/` — entry-point scripts, each with an exported `async function main(ns: NS)`
  ...
  - `home-ram-loop.ts` / `home-cores-loop.ts` / `backdoor-loop.ts` / `company-work-loop.ts` / `battlestation.ts` — exist but are **not** chain-launched: the first four hard-require Source-File 4 (unowned on this save), `battlestation.ts` is a manual-use HUD. Run by hand.
...
- **Fixed a RAM-starvation bug leaving rootable hosts stuck `[locked]`**: `controller.ts` reserved home RAM for every *persistent* manager's own launch, but never for `scan-root.js` — the transient 3.80GB script `rescan-loop.js` spawns fresh every ~30s while staying resident itself, so nothing protected that recurring child spawn from dispatch's greedy weaken/grow/hack RAM claim. Fixed by adding a `scanRootReserveGb` term to `controller.ts`'s reserve calculation, same `isRunning`-gated pattern as the other reserves. Confirmed live: the "Cannot run scripts/scan-root.js... requires 3.80GB" spam in `rescan-loop.js`'s tail stopped after the fix.
- **`home-upgrade-loop.ts` split into `home-ram-loop.ts` / `home-cores-loop.ts`**: each now loops a single `singularity.upgradeHomeRam()`/`upgradeHomeCores()` call instead of both, so a manual run only needs to pay for the one upgrade actually wanted (roughly half the combined script's confirmed 99.65GB, per the same doc-comment cost math). Still not chain-launched — both still hard-require Source-File 4.
