...
- **`faction-work-loop.ts` now rotates reputation-earning across every joined faction instead of committing to one forever.** Traced from a user report that the augment purchaser kept installing NeuroFlux Governor-only batches — a first fix (gate NFG purchases on a real augment already queued) was a real improvement but didn't resolve it. A diagnostic tail line (`candidates=N gated=N budget=$N`) showed the actual state: 60 real augmentations existed, unowned, blocked purely by insufficient faction reputation, with $2.9B sitting idle. Root cause was `faction-work-loop.ts` picking one faction on first success and never revisiting it. Now it re-ranks all joined factions every tick by reputation-gap to their nearest unowned augmentation and works whichever is closest, rolling onto the next as each gap closes. `ram-audit` caught two real bugs in the new code before it synced (a `NaN` from `Infinity - Infinity` in the sort comparator, and a `??` operator that trips this project's own confirmed phantom-RAM-charge finding) — both fixed pre-push. Not yet confirmed live in-game.
