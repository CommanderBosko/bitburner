...
- `src/scripts/` — entry-point scripts, each with an exported `async function main(ns: NS)`
  ...
  - `controller.ts` / `battlestation.ts` / `hacknet-manager.ts` / `rescan-loop.ts` / `backdoor-loop.ts` / `darknet-manager.ts` — the rest of the persistent automation chain, each launching the next
...
- Reverted `hacknet-manager.ts`'s 30-minute payback-period cap (it stalled purchasing entirely as costs grew) and switched its gain-rate calculation to the real `ns.formulas.hacknetNodes.moneyGainRate()` now that `Formulas.exe` is owned.
