...
- Fixed two real bugs in `controller.ts`'s `buildWorkingSet` that were leaving hacking scripts earning $0 despite a large purchased-server fleet: it was debiting its simulated RAM pool by an unbounded demand ceiling instead of a candidate's real one-batch cost, and using `break` instead of `continue` when a candidate couldn't be funded — together these let one expensive, top-scored target lock every cheaper, easily-fundable target out of the working set. Also added `ns.disableLog("ALL")` plus permanent diagnostic prints so a future RAM-starvation stall shows up directly in the tail log.
- Fixed a related set of RAM-dispatch bugs the session before: prep-phase weaken is now sized off the RAM-capped, actually-fundable grow thread count instead of an uncapped desired count, and `home-ram-loop.ts`'s launch is gated on `rescan-loop.ts` already running to avoid a boot race.
...
- `server-purchase-manager.ts` no longer waits behind the 64GB home-RAM gate — purchased-server RAM feeds the same host pool dispatch draws from, so it helps hacking rather than competing with it, unlike `hacknet-manager.ts`/`battlestation.ts` which remain gated.
