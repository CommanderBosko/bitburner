...
import type { DarknetDiscoveryMessage, DarknetHopFailedMessage } from "../lib/darknet-ports";
...
export async function main(ns: NS): Promise<void> {
	...
	if (remainingPath.length > 0) {
		...
		try {
			...
			ns.print(`darknet-agent-recon: hop to ${next.host} failed (path likely stale) - ${String(error)}`);
